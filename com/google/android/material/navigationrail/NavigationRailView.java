...
public class NavigationRailView extends NavigationBarView {
    ...
    public NavigationRailView(Context context, AttributeSet attrs) {
        this(context, attrs, C0117R.attr.navigationRailStyle);
    }

    public NavigationRailView(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, C0117R.style.Widget_MaterialComponents_NavigationRailView);
    }
    ...
    public NavigationRailView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.topMargin = getResources().getDimensionPixelSize(C0117R.dimen.mtrl_navigation_rail_margin);
        TintTypedArray attributes = ThemeEnforcement.obtainTintedStyledAttributes(getContext(), attrs, C0117R.styleable.NavigationRailView, defStyleAttr, defStyleRes, new int[0]);
        int headerLayoutRes = attributes.getResourceId(C0117R.styleable.NavigationRailView_headerLayout, 0);
        if (headerLayoutRes != 0) {
            addHeaderView(headerLayoutRes);
        }
        setMenuGravity(attributes.getInt(C0117R.styleable.NavigationRailView_menuGravity, 49));
        if (attributes.hasValue(C0117R.styleable.NavigationRailView_itemMinHeight)) {
            setItemMinimumHeight(attributes.getDimensionPixelSize(C0117R.styleable.NavigationRailView_itemMinHeight, -1));
        }
        if (attributes.hasValue(C0117R.styleable.NavigationRailView_paddingTopSystemWindowInsets)) {
            this.paddingTopSystemWindowInsets = Boolean.valueOf(attributes.getBoolean(C0117R.styleable.NavigationRailView_paddingTopSystemWindowInsets, false));
        }
        if (attributes.hasValue(C0117R.styleable.NavigationRailView_paddingBottomSystemWindowInsets)) {
            this.paddingBottomSystemWindowInsets = Boolean.valueOf(attributes.getBoolean(C0117R.styleable.NavigationRailView_paddingBottomSystemWindowInsets, false));
        }
        attributes.recycle();
        applyWindowInsets();
    }
